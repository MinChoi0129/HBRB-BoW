...
public:
   ...
   * @param kmeans_type type of kmeans algorithm to use
   */
...
protected:
  ...
  /**
   * Recursively creates the tree using real-valued descriptors (Method 2)
   * Stores the real-valued centers in a map temporarily.
   */
  void HKmeansStepReal(NodeId parent_id, const cv::Mat &all_descriptors,
                       const std::vector<unsigned int> &indices, int current_level,
                       std::map<NodeId, cv::Mat> &real_centers);
...
void TemplatedVocabulary<TDescriptor, F>::create(
    const std::vector<std::vector<TDescriptor>> &training_features, int kmeans_type) {

  std::cout << "Creating vocabulary with kmeans_type: " << kmeans_type << std::endl;
  ...
  std::cout << "Reserving " << expected_nodes << " nodes" << std::endl;
  ...
  if (kmeans_type == 0 || kmeans_type == 1) {
    std::vector<pDescriptor> features;
    std::cout << "Getting features" << std::endl;
    getFeatures(training_features, features);

    std::cout << "Creating tree" << std::endl;
    HKmeansStep(0, features, 1, kmeans_type);
  } else if (kmeans_type == 2) {
    std::cout << "[Method 2] Global B2R -> Real-space Tree -> Global R2B" << std::endl;

    // 1. Flatten features
    std::vector<TDescriptor> all_features_flat;
    size_t total_count = 0;
    for (const auto &v : training_features)
      total_count += v.size();
    all_features_flat.reserve(total_count);

    for (const auto &v : training_features) {
      all_features_flat.insert(all_features_flat.end(), v.begin(), v.end());
    }

    if (all_features_flat.empty())
      return;

    // 2. Convert B2R once
    cv::Mat all_data_f;
    F::toMat32F(all_features_flat, all_data_f);

    // 3. Prepare indices
    std::vector<unsigned int> indices(all_data_f.rows);
    std::iota(indices.begin(), indices.end(), 0);

    // 4. Build Tree in Real Space
    std::cout << "Building tree in real-space..." << std::endl;
    std::map<NodeId, cv::Mat> real_centers_map;
    HKmeansStepReal(0, all_data_f, indices, 1, real_centers_map);

    // 5. Global R2B Conversion
    std::cout << "Converting all nodes to Binary..." << std::endl;
    const int dim = all_data_f.cols;

    for (auto const &[nid, center_f] : real_centers_map) {
      TDescriptor bin_desc = cv::Mat::zeros(1, F::L, CV_8U);
      unsigned char *p = bin_desc.template ptr<unsigned char>();

      for (int byte_idx = 0; byte_idx < F::L; ++byte_idx) {
        unsigned char byte_val = 0;
        const int base_dim = byte_idx * 8;
        for (int bit = 0; bit < 8 && (base_dim + bit) < dim; ++bit) {
          const float v = center_f.at<float>(0, base_dim + bit);
          if (v >= 0.5f) {
            byte_val |= static_cast<unsigned char>(1 << (7 - bit));
          }
        }
        p[byte_idx] = byte_val;
      }
      m_nodes[nid].descriptor = bin_desc;
    }
  }
...
void TemplatedVocabulary<TDescriptor, F>::getFeatures(
  ...
  const size_t total_size = training_features.size();
  ...
  for (vvit = training_features.begin(); vvit != training_features.end(); ++vvit) {
    ...
    current_idx++;
    if (total_size > 0) {
      int progress = (int)((current_idx * 100) / total_size);
      std::cout << "\rProcessing features: " << progress << "%" << std::flush;
    }
  }
...
void TemplatedVocabulary<TDescriptor, F>::HKmeansStep(NodeId parent_id,
                                                      ...
                                                      int current_level, int kmeans_type) {
  ...
  // 2. Method 1: Local BRB (BRB-KMeans per node)
  else if (kmeans_type == 1) {
    // (1) Collect descriptors & B2R
    std::vector<TDescriptor> vec_descriptors;
    vec_descriptors.reserve(descriptors.size());
    for (size_t i = 0; i < descriptors.size(); ++i) {
      vec_descriptors.push_back(*descriptors[i]);
    }

    cv::Mat data;
    F::toMat32F(vec_descriptors, data); // Binary -> Float

    // (2) Run K-Means (Real-Space)
    cv::Mat labels;
    cv::Mat centers; // Float Centers
    cv::TermCriteria criteria(cv::TermCriteria::MAX_ITER + cv::TermCriteria::EPS, 300, 1e-4);

    // Use KMEANS_PP_CENTERS for fair comparison with Method 2
    cv::kmeans(data, m_k, labels, criteria, 1, cv::KMEANS_PP_CENTERS, centers);

    // (3) R2B: Convert Float Centers to Binary IMMEDIATELLY
    clusters.clear();
    const int dim = data.cols;
    for (int c = 0; c < m_k; ++c) {
      TDescriptor center_desc = cv::Mat::zeros(1, F::L, CV_8U);
      unsigned char *p = center_desc.template ptr<unsigned char>();

      for (int byte_idx = 0; byte_idx < F::L; ++byte_idx) {
        unsigned char byte_val = 0;
        const int base_dim = byte_idx * 8;
        for (int bit = 0; bit < 8 && (base_dim + bit) < dim; ++bit) {
          const float v = centers.at<float>(c, base_dim + bit);
          if (v >= 0.5f) {
            byte_val |= static_cast<unsigned char>(1 << (7 - bit));
          }
        }
      ...
      clusters.push_back(center_desc);
    }

    // (4) Re-assignment based on Hamming Distance
    // *중요*: cv::kmeans의 labels(Euclidean 기준)를 쓰지 않고,
    // 이진화된 Centroid와의 Hamming 거리로 데이터를 다시 분배합니다.
    // 이것이 Method 2(Real 유지)와의 차이점을 만듭니다.

    for (auto &g : groups)
      g.clear();

    for (size_t i = 0; i < descriptors.size(); ++i) {
      const TDescriptor &d = *descriptors[i];
      double best_dist = F::distance(d, clusters[0]);
      unsigned int best_idx = 0;

      for (int c = 1; c < m_k; ++c) {
        double dist = F::distance(d, clusters[c]);
        if (dist < best_dist) {
          best_dist = dist;
          best_idx = c;
        }
      }
      groups[best_idx].push_back(i);
    }
  }
  // 3. Method 0: K-Majority (Original DBoW2 implementation)
  else {
    ...
    while (goon) {
      ...
      if (first_time) {
        ...
        for (unsigned int c = 0; c < clusters.size(); ++c) {
          ...
          for (unsigned int idx : groups[c]) {
            cluster_descriptors.push_back(descriptors[idx]);
          }
          if (!cluster_descriptors.empty())
            F::meanValue(cluster_descriptors, clusters[c]); // k-majority mean (bit vote)
        }
      }

      // (2) Associate features (Hamming Distance)
      groups.assign(clusters.size(), std::vector<unsigned int>());
      current_association.resize(descriptors.size());

      typename std::vector<pDescriptor>::const_iterator fit;
      size_t idx = 0;
      for (fit = descriptors.begin(); fit != descriptors.end(); ++fit, ++idx) {
      ...
      if (first_time) {
        ...
        goon = false;
        for (size_t i = 0; i < current_association.size(); i++) {
  ...
  if (current_level < m_L) {
    ...
    for (unsigned int i = 0; i < clusters.size(); ++i) {
      ...
      for (unsigned int idx : groups[i]) {
        child_features.push_back(descriptors[idx]);
      }

      if (child_features.size() > 1) {
        HKmeansStep(id, child_features, current_level + 1, kmeans_type);
      }
    }
  }
}
...
template <class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor, F>::HKmeansStepReal(NodeId parent_id,
                                                          const cv::Mat &all_descriptors,
  ...
  cv::Mat current_data(indices.size(), all_descriptors.cols, all_descriptors.type());
  for (size_t i = 0; i < indices.size(); ++i) {
    all_descriptors.row(indices[i]).copyTo(current_data.row(i));
  }

  // 2. Run K-Means (Real-space)
  int k = m_k;
  if ((int)indices.size() <= m_k) {
    k = (int)indices.size();
  }

  cv::Mat labels;
  cv::Mat centers; // Resulting centers in Float

  if (k == (int)indices.size()) {
    centers = current_data;
    labels = cv::Mat::zeros(k, 1, CV_32S);
    for (int i = 0; i < k; ++i)
      labels.at<int>(i) = i;
  } else {
    cv::TermCriteria criteria(cv::TermCriteria::MAX_ITER + cv::TermCriteria::EPS, 300, 1e-4);
    cv::kmeans(current_data, k, labels, criteria, 1, cv::KMEANS_PP_CENTERS, centers);
  }

  // 3. Create Nodes Structure (Topology only)
  // R2B 변환을 여기서 수행하지 않고, Float Center를 map에 저장합니다.

  std::vector<std::vector<unsigned int>> groups(k);
  for (int i = 0; i < labels.rows; ++i) {
    int cluster_idx = labels.at<int>(i);
    if (cluster_idx >= 0 && cluster_idx < k) {
      groups[cluster_idx].push_back(indices[i]);
    }
  }
  ...
  for (int i = 0; i < k; ++i) {
    ...
    // Save float center for later conversion
    // centers.row(i) returns a temporary header, so we clone it to store.
    real_centers[id] = centers.row(i).clone();
  }
  ...
  if (current_level < m_L) {
    ...
    for (int i = 0; i < k; ++i) {
      NodeId id = children_ids[i];
      if (groups[i].size() > 0) {
        HKmeansStepReal(id, all_descriptors, groups[i], current_level + 1, real_centers);
      }
    }
  }
}
...
void TemplatedVocabulary<TDescriptor, F>::saveToBinaryFile(const std::string &filename) const {
  ...
  unsigned int size_node =
      sizeof(m_nodes[0].parent) + F::L * sizeof(char) + sizeof(_weight) + sizeof(bool);
...
void TemplatedVocabulary<TDescriptor, F>::load(const std::string &filename) {
  if (!loadFromBinaryFile(filename)) {
    throw std::string("Could not load vocabulary from file ") + filename;
  }
}
